...
Kaatein::Kaatein(float x, float y, color_t color,float size)
{
    this->rotation = 0;
    ...
    GLfloat vertex_buffer_data[27];

    int i,j;
    for(i=0; i<n; i++)
    {
        float ab=theta*i+1.5707;
        float angle=ab+theta+1.5707;
...
void Kaatein::set_position(float x, float y)
{
    speedx=0.01;
    this->position = glm::vec3(x, y, 0);
}
void Kaatein::change_speed()
{
    speedx  += accx ;
    speedy  += accy ;
}
...
bounding_box_t Kaatein::bounding_box()
{
